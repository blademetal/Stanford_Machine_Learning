...
% Add a first column of 1's to matrix X
X = [ones(size(X, 1), 1) X];
...
J = (1/m)*sum(sum(-y_matrix .* log(sigmoid(z2*Theta2')) - (1 .- y_matrix).*log(1 - sigmoid(z2*Theta2'))));

% Regularized cost function
Theta1_r = Theta1(:,2:size(Theta1, 2));
Theta2_r = Theta2(:,2:size(Theta2, 2));

regularization = (lambda/(2*m))*(sum(sum(Theta1_r.**2))+sum(sum(Theta2_r.**2)));
J = J + regularization;


a_3 = sigmoid(z2*Theta2');
...
d_2 = (Theta2_back'*d_3')'.*sigmoidGradient(z2);
